 ...
 * ステージ選択5列化・縦スクロールバー設置・スマホなぞり入力完全修正版
 */
...
const PicrossGame = {
    ...
    setupStageData() {
        ...
        for (let i = 0; i < size * size; i++) {
            this.answerGrid.push(parseInt(rawStr[i], 10));
        }
...
Object.assign(PicrossGame, {
    render() {
        ...
        cornerCell.style.minHeight = "45px"; 
        ...
        for (let c = 0; c < size; c++) {
            ...
            colHintCell.style.minHeight = "45px";           
            colHintCell.style.padding = "4px 0";           
        ...
        for (let r = 0; r < size; r++) {
            ...
            for (let c = 0; c < size; c++) {
                const idx = r * size + c;
                const cellState = this.playerGrid[idx];
    ...
    startDragInput(index, r, c) {
        const currentState = this.playerGrid[index];
        ...
        if (this.currentTool === "fill") {
            if (currentState === 0) {
                this.dragOperationMode = "fill_black"; 
            } else if (currentState === 1) {
                this.dragOperationMode = "delete_black"; 
            } else {
                this.dragOperationMode = "none"; 
            }
        } 
            ...
            if (currentState === 0) {
                this.dragOperationMode = "fill_batsu"; 
            } else if (currentState === 2) {
                this.dragOperationMode = "delete_batsu"; 
            } else {
                this.dragOperationMode = "none"; 
            }
        } else {
    ...
    executeDragInput(index) {
        ...
        if (nextState !== currentState) {
            this.playerGrid[index] = nextState;
            this.render();
    ...
    checkWinCondition() {
        const isWin = this.playerGrid.every((state, idx) => {
            const answer = this.answerGrid[idx];
...
function executeSwitchPicrossMode() {
    ...
    if (picrossMode === "fixed") {
        ...
        // 👇【追加】ステージ選択画面の時は、盤面コンテナの中身を完全に空にして余白を消し去る
        const boardContainer = document.getElementById("picross-board-container");
        if (boardContainer) {
            boardContainer.innerHTML = "";
            boardContainer.style.border = "none"; // 枠線の余白も消す
...
function executeBackToPicrossStageSelect() {
    ...
    // 👇【追加】ここでもステージ選択に戻った時に盤面コンテナの見た目の余白を完全にリセットする
    const boardContainer = document.getElementById("picross-board-container");
    if (boardContainer) {
        boardContainer.innerHTML = "";
        boardContainer.style.border = "none";
        boardContainer.style.padding = "0";
    }
...
function renderPicrossStageSelect() {
    ...
    const levelsJa = ["初級(5x5)", "中級(10x10)"]; 
    
    const titleEl = document.getElementById('picross-stage-title');
    if (titleEl) {
        titleEl.innerText = `${levelsJa[currentPicrossLevelIndex]} ステージ選択 (${start}〜${start+99})`;
    }

    let clearedStages = typeof getClearedStages === 'function' ? getClearedStages('picross_cleared_stages') : [];
    // renderPicrossStageSelect() メソッド内のループ処理の部分を以下のようにクラス名修正
    for (let i = start; i <= start + 99; i++) {
        ...
        if (hasProblem) {
            ...
            // 新しい共通クラス名と非活性デザインに変更
            btn.className = 'stage-btn-common disabled';
            btn.innerText = "-"; 
            btn.disabled = true;
        }
